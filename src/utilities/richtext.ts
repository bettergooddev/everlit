...
/**
 * Check if rich text content is empty (contains no meaningful text)
 * @param content - The rich text content to check
 * @returns true if the content is empty or contains only whitespace
 */
export function isRichTextEmpty(content: any): boolean {
  if (!content) return true

  const plainText = extractPlainText(content, Infinity) // No length limit for empty check
